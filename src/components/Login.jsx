...
const Login = ()=> {
  ...
  const navigate = useNavigate()
   ...
   const {viewPassword, setViewPassword, isLoading, setIsLoading, email, setEmail, password, setPassword} = useContext(AppContext)
    ...
    const handleLogin = async (e)=> {
      ...
      try{
        ...
        if(response.data){
          ...
          localStorage.setItem('current_session', Date.now())
          sessionStorage.setItem('my_session', Date.now())
    ...
    <div className="flex items-center justify-center h-screen w-full bg-white">
      <div className="w-full flex flex-col max-w-300 mx-auto gap-5 md:flex-row bg-white shadow shadow-slate-300">
        ...
        <div className="flex-1 w-full p-1 rounded-2xl bg-white md:p-5">
          <form className="flex flex-col gap-8 p-5" onSubmit={handleLogin} method='POST'>
            <div className="text-center text-3xl font-bold"><h3>USER LOGIN</h3></div>
            ...
            <div className='relative flex items-center w-full transform group'>
              <button type="submit" className="flex items-center justify-center p-3 bg-blue-800 text-3xl rounded-xl text-white shadow shadow-slate-600 w-full transition-transform duration-200 group-hover:scale-101">
                {isLoading ? <Loader2 size={35} className='animate-spin'/> : 'Login'}
              </button>
              <SendHorizonal size={24} className={`absolute right-[22%] transition-all text-white group-hover:right-[20%] md:right-[35%] md:group-hover:right-[33%] ${isLoading && 'hidden'} `}/>
            </div>
            <div>
            <div className='flex items-center gap-2 text-slate-600'>
                Dont have account? 
              <Link to='/' className='text-blue-900'>Register</Link>
            </div>
          </div>
          </form>
        </div>
