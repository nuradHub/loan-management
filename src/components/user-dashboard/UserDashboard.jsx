...
const UserDashboard = () => {
  ...
  const { selectedMenu, isLoan, setIsLoan, setIsPayment, loans, setIsMenuOpen, currentUser, handleCurrentLoan, setSelectedMenu, isPayment, isEligibility, setIsEligibility, handleApprovedAmount, approvedAmount, creditScore } = useContext(AppContext)
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="lg:col-span-2 bg-[#1E293B] rounded-2xl p-6 text-white shadow-lg flex flex-col justify-between">
                  <div>
                    ...
                    <h2 className="text-3xl font-bold mt-1 tracking-tight">₦{(approvedAmount ?? 0).toFixed(2)}</h2>
                  </div>
                    ...
                    {[...loans]?.sort((a, b)=> new Date(b.createdAt) - new Date(a.createdAt)).map((loan) => (
                      <div key={loan._id} className="p-3 bg-slate-50 rounded-xl flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          <div className="h-8 w-8 rounded-lg bg-blue-100 text-blue-600 flex items-center justify-center font-bold text-xs italic">$</div>
                          <div>
                        ...
                        <span className="text-[10px] font-bold text-blue-600">{loan?.status}</span>
                      </div>
                    ))}
                  </div>
                </section>
                  ...
                  {[...loans]?.sort((a, b)=> new Date(b.createdAt) - new Date(a.createdAt)).map((loan) => (
                    <div key={loan._id} className="bg-white p-5 rounded-2xl border border-slate-200 hover:shadow-md transition-all">
                      <div className="flex justify-between items-start mb-4">
                        ...
                        <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-green-50 text-green-600 uppercase tracking-tighter">{loan?.status}</span>
                      </div>
                      ...
                      <div className="pt-4 border-t flex justify-between items-center">
                          ...
                          <span className="text-lg font-bold text-slate-800">₦{loan?.amount}</span>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div className="max-w-2xl bg-white rounded-2xl border border-slate-200 shadow-sm animate-fadeIn overflow-hidden">
                <div className="h-24 bg-slate-100 border-b relative px-8 flex items-end">
                    <div className="h-16 w-16 bg-white p-1 rounded-xl translate-y-6 shadow-md border">
                        <div className="h-full w-full bg-slate-800 rounded-lg flex items-center justify-center text-white"><UserCircle size={30}/></div>
                    </div>
                </div>
                <div className="p-8 pt-10 flex flex-col gap-6">
                    <div>
                        ...
                        <p className="text-xs font-medium text-slate-400">Personal Account • Joined {currentUser?.createdAt}</p>
                    </div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 border-t pt-6">
                        ...
                        <div className="p-3 bg-slate-50 rounded-xl">
                            ...
                            <p className="text-xs font-bold text-slate-700">{currentUser?.tel ? currentUser?.tel : ''}</p>
                        </div>
                    </div>
                    <button className="bg-slate-800 text-white py-2 rounded-xl text-xs font-bold w-fit px-6 hover:bg-black transition-all">Edit Account</button>
                </div>
            </div>
          )}
